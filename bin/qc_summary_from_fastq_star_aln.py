...
def getQC(rnaID):
  ...
  alnsummary = rnaID + '.Log.final.out'
  ...
  qc_data['uniqMapPercent']=str([x for x in open(alnsummary) if x.strip().startswith('Uniquely mapped reads % |')][0].split()[-1]).replace('%','')
