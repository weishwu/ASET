...
vcf_fn = sys.argv[2]
ase = sys.argv[3]
ase_hetsnp_out_fn = sys.argv[4]
ase_homsnp_out_fn = sys.argv[5]
ase_homref_out_fn = sys.argv[6]
...
ASE=pd.read_csv(ase,sep="\t",dtype=str).set_axis(['variantID','refCount','altCount','reffreq'], axis=1).drop('reffreq',axis=1)
ASE['totalCount'] = ASE['refCount'] + ASE['altCount']
ASE['strand'] = "."
ASE['RNAid'] = sample_id

hetsnpASE=hetsnp.merge(ASE,on='variantID')
homsnpASE=homsnp.merge(ASE,on='variantID')
homrefASE=homref.merge(ASE,on='variantID')
